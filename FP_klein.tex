...
\begin{document}
	...
	\includegraphics[width=16cm,height=40cm,keepaspectratio]{/home/lisa/WS2526/FP/pipeline_numbered.png}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{>{\bfseries}p{0.8cm} p{4cm} p{8cm}}
			...
			2 & Data Table (1) & Inspect the imported data in tabular form. \\
			3 & Formula & Optionally create or adjust variables (e.g., derive new numeric features). \\
			4 & Distances (3) & Compute pairwise distances or similarities between coin images. \\
			5 & Hierarchical Clustering (search\_all) & Perform clustering on the dataset using all available data. (Developed by a collaborator of the supervising team.) \\
			6 & Hierarchical Clustering (3) & Optional alternative clustering with different parameters or subsets. \\
			7 & Image Viewer & Visually inspect which images belong to each cluster for clustering with search\_all. \\
			8 & Python Script & Analyze the clusters by counting obverse and reverse images to assess clustering quality. \\
			9 & Image Viewer (5) & Review cluster assignments from optional alternative clustering (3) \\
			10 & Data Table (2) & Review the cluster assignments and their quality in tabular form. \\
			11 & Save Data & Save the clustered dataset and their quality. \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	In the \textit{Variable Definitions} field, a new variable (for example path\_1) is created.  
	...
	Different distance metrics can be selected depending on the research focus, for example, Cosine or Pearson (absolute) (see Section~\ref{sec:metrics} for details).
	\\\\
	\includegraphics[width=6cm,height=15cm,keepaspectratio]{/home/lisa/WS2526/FP/distance.png}
	...
	It was developed by a collaborator of the supervising research group.  
	...
	The standard \textbf{Hierarchical Clustering} widget can be used to run alternative clustering configurations or to compare parameter settings.  
	It provides access to linkage methods such as \textbf{Average} or \textbf{Complete} linkage and allows the user to interactively adjust the number of clusters.
	
	\subsection*{Image Viewer}
	The \textbf{Image Viewer} widget visualizes the images contained in a selected cluster.  
	This helps to evaluate whether the clustering corresponds to numismatic expectations — for example, whether the dies, styles, or motifs appear related.  
	...
	The \textbf{Save Data} widget stores the clustered dataset to disk for later reuse or external analysis.  
	This ensures reproducibility and allows further evaluation in other tools.
	
	\subsection*{Python Script}
	The \textbf{Python Script} widget contains the custom script that evaluates the internal consistency of each cluster.  
	It counts how many obverse (\texttt{o.jpg}) and reverse (\texttt{r.jpg}) images are present and calculates a quality score based on their ratio.  
	...
	\subsection*{Data Table (Output)}
	The final \textbf{Data Table (Output)} displays the calculated values — the number of front and back images per cluster, total counts, and the global average quality.  
	...
	\section{Distance Metrics and Linkage Methods}
	...
	Cosine distance measures the \textbf{angle between image feature vectors}, focusing on their orientation rather than magnitude.  
	...
	Pearson correlation distance compares the \textbf{linear relationship} between two feature vectors.  
	...
	The clustering step groups coin images based on visual similarity of die features, while the subsequent quality evaluation checks whether obverse and reverse images were accidentally mixed.  
	The resulting values allow researchers to quickly estimate the reliability of the clustering and to understand how parameter choices (distance metric and linkage) influence the outcome.
	
	Clusters containing only obverse or only reverse images indicate high internal consistency and are more likely to represent coins struck from the same or related dies.  
	Mixed clusters, on the other hand, suggest over-broad grouping or unsuitable parameter settings.
	
	%------------------------------------------------
	\section{Appendix – Technical Note}
	The Python Script widget uses a custom Orange Data Mining script that:
	\begin{enumerate}
		\item Groups all images by their cluster ID.
		\item Counts the number of front/back images in each group.
		\item Computes per-cluster and global quality metrics.
		\item Outputs a summary table for visualization and export.
	\end{enumerate}
	
	%------------------------------------------------
	\section*{Summary}
	This workflow provides an accessible method to cluster and evaluate coin images based on die similarity.  
	By combining hierarchical clustering, visual inspection, and an automated quality score, it helps numismatists assess dataset consistency and test how clustering parameters affect their results.
